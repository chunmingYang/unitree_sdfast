...
#define lasterr (a1gerror.lasterr_)
#define lastrou (a1gerror.lastrou_)
    ...
    int errnum)
{
    ...
    if (routine == 8) {
        fprintf(fnum,"a1state:\n");
    }
    if (routine == 9) {
        fprintf(fnum,"a1psstate:\n");
    }
    if (routine == 13) {
        fprintf(fnum,"a1presacc:\n");
    }
    if (routine == 14) {
        fprintf(fnum,"a1presvel:\n");
    }
    if (routine == 15) {
        fprintf(fnum,"a1prespos:\n");
    }
    if (routine == 10) {
        fprintf(fnum,"a1hinget:\n");
    }
    if (routine == 11) {
        fprintf(fnum,"a1pointf:\n");
    }
    if (routine == 12) {
        fprintf(fnum,"a1bodyt:\n");
    }
    if (routine == 17) {
        fprintf(fnum,"a1deriv:\n");
    }
    if (routine == 16) {
        fprintf(fnum,"a1resid:\n");
    }
    if (routine == 18) {
        fprintf(fnum,"a1pseudo:\n");
    }
    if (routine == 19) {
        fprintf(fnum,"a1mom:\n");
    }
    ...
    if (routine == 23) {
        fprintf(fnum,"a1orient:\n");
    }
    if (routine == 24) {
        fprintf(fnum,"a1angvel:\n");
    }
    if (routine == 25) {
        fprintf(fnum,"a1trans:\n");
    }
    if (routine == 26) {
        fprintf(fnum,"a1perr:\n");
    }
    ...
    if (routine == 28) {
        fprintf(fnum,"a1psqdot:\n");
    }
    if (routine == 29) {
        fprintf(fnum,"a1psudot:\n");
    }
    if (routine == 30) {
        fprintf(fnum,"a1getht:\n");
    }
    if (routine == 31) {
        fprintf(fnum,"a1reac:\n");
    }
    ...
    if (routine == 39) {
        fprintf(fnum,"a1getgrav:\n");
    }
    if (routine == 40) {
        fprintf(fnum,"a1getmass:\n");
    }
    if (routine == 41) {
        fprintf(fnum,"a1getiner:\n");
    }
    if (routine == 42) {
        fprintf(fnum,"a1getbtj:\n");
    }
    if (routine == 43) {
        fprintf(fnum,"a1getitj:\n");
    }
    if (routine == 44) {
        fprintf(fnum,"a1getpin:\n");
    }
    if (routine == 45) {
        fprintf(fnum,"a1getpres:\n");
    }
    if (routine == 46) {
        fprintf(fnum,"a1getstab:\n");
    }
    if (routine == 47) {
        fprintf(fnum,"a1info:\n");
    }
    ...
    if (routine == 50) {
        fprintf(fnum,"a1assemble:\n");
    }
    if (routine == 51) {
        fprintf(fnum,"a1initvel:\n");
    }
    if (routine == 52) {
        fprintf(fnum,"a1static:\n");
    }
    if (routine == 53) {
        fprintf(fnum,"a1steady:\n");
    }
    if (routine == 54) {
        fprintf(fnum,"a1motion:\n");
    }
    if (routine == 55) {
        fprintf(fnum,"a1fmotion:\n");
    }
    if (routine == 56) {
        fprintf(fnum,"a1equivht:\n");
    }
    if (routine == 57) {
        fprintf(fnum,"a1massmat:\n");
    }
    if (routine == 58) {
        fprintf(fnum,"a1frcmat:\n");
    }
    if (routine == 59) {
        fprintf(fnum,"a1rel2cart:\n");
    }
    if (routine == 60) {
        fprintf(fnum,"a1comptrq:\n");
    }
    if (routine == 61) {
        fprintf(fnum,"a1fulltrq:\n");
    }
    if (routine == 62) {
        fprintf(fnum,"a1vrot:\n");
    }
    ...
    if (errnum == 22) {
        fprintf(fnum,"a1init must be called first\n");
    }
    if (errnum == 23) {
        fprintf(fnum,"a1state must be called first\n");
    }
    if (errnum == 24) {
        fprintf(fnum,"a1deriv must be called first\n");
    }
    if (errnum == 25) {
...
void a1printerr(FILE *fnum)
{

    a1prerrmsg(fnum,lastrou,lasterr);
}

void a1clearerr(void)
{
    ...
    double x[])
{
    ...
    a1ldubsl(n,na,map,l,b,work);
    a1ldubsd(n,na,map,d,work,work);
    a1ldubsu(n,na,map,l,work,x);
}
    ...
    double x[])
{
    ...
    a1ldudcomp(n,na,map,tol,work1,work2,m,l,d);
    a1ldubslv(n,na,map,work2,l,d,b,x);
}
    ...
    int *rank)
{
    ...
    if (k  !=  0) {
        a1qrsl(nr,nc,nra,nca,mapr,mapc,k,work,w,qraux,b,x);
    }
    for (j = k; j <= nca-1; j++) {
        x[mapc[j]] = 0.;
    }
    ...
    int *rank)
{
    ...
    a1qrdcomp(nr,nc,nra,nca,mapr,mapc,w,qraux,jpvt);
    a1qrbslv(nr,nc,nra,nca,mapr,mapc,tol,work,iwork,w,qraux,jpvt,b,x,rank);
}
    ...
    double x[])
{
    ...
    if (ndes  ==  0) {
        ...
        a1qrslv(nra,nra,nra,nra,&iw[map],&iw[map],tol,&iw[jpvt],&rw[qraux],&rw[
        ...
        a1qrslv(dsiz,dsiz,dsiz,dsiz,&iw[map],&iw[map],tol,&iw[jpvt],&rw[qraux],&
...
/* Utility routines for use with a1root. */


void a1calcerrs(double fval[],
    int nfunc,
    int ndes,
    int dnorm,
    double *maxderr,
    ...
    double newerrs[])
{
    ...
    *fcnt = *fcnt+1;
    ...
    a1calcerrs(newerrs,nfunc,ndes,dnorm,&maxderr,&maxrerr,&derrnorm);
    ...
    int *err)
{
    ...
    *fcnt = *fcnt+1;
    a1calcerrs(fret,nfunc,ndes,dnorm,&pmaxderr,&pmaxrerr,&pderrnorm);
    crude = 1e-6;
    for (ntrys = 0; ntrys < 2; ntrys++) {
        ...
        for (;;) {
            ...
            a1calcjac(func,vars,param,nfunc,nvar,lock,crude,fret,&rw[f1]
              ,jw,fcnt,&rw[scale]);
            for (i = 0; i < nfunc; i++) {
                rw[f1+i] = fret[i]*rw[scale+i];
            }
            a1lsslv(nfunc,nvar,nfunc,nvar,ndes,&iw[mapf],&iw[mapv],qrtol,dw,&rw[
              morerw],&iw[moreiw],jw,&rw[f1],&rw[deltav]);
            for (;;) {
                a1adjvars(func,vars,param,nfunc,ndes,dnorm,nvar,&rw[deltav]
                  ,step,pmaxrerr,pderrnorm,rtol,fcnt,&rw[guess],&rw[f1]);
                a1calcerrs(&rw[f1],nfunc,ndes,dnorm,&maxderr,&maxrerr,&derrnorm)
                  ;
...
/* Utility routine for use with a1finteg and a1vinteg.  Work is 2*neq. */


void a1rk4m(int (*func)(),
    double time,
    double st[],
    double dst0[],
    double param[],
    ...
    int *status)
{
    ...
    ttime = *time;
    if (step > 0.) {
        a1rk4m(func,ttime,st,dst,param,step,&work[nst],neq,&work[morework],&work
          [errs],errest,&which);
        for (i = 0; i < neq; i++) {
            st[i] = work[nst+i];
        }
    ...
    int *which)
{
    ...
    *err = 0;
    ...
    while (ttime  <  tfin) {
        ...
        nstp = stp;
        for (;;) {
            a1rk4m(func,ttime,st,dst,param,nstp,&work[xst],neq,&work[morework],&
              work[errs],&maxerr,&wh);
            xtime = ttime+nstp;
            if (maxerr <= tol) {
                errf = 0;
            ...
            if (stp2 < minstep) {
                ...
                a1rk4m(func,xtime,&work[xst],&work[xdst],param,stp2,&work[xst2]
...
void a1dc2ang(double dircos[3][3],
    double *a1,
    double *a2,
    double *a3)
{
...
void a1dc2quat(double dircos[3][3],
    double *e1,
    double *e2,
    double *e3,
    double *e4)
...
double 
a1vnorm(double ivec[3])
{
    double norm;

/* Compute the length (norm) of a vector
*/
    norm = sqrt(a1vdot(ivec,ivec));
    return norm;
}

void a1vcopy(double ivec[3],
    double ovec[3])
{
...
*/
    ...
    stheta = sin(theta);
    ctheta = cos(theta);
    a1vmul(stheta,ivec,isinvec);
    a1vmul(ctheta,ivec,icosvec);
    norm = 1./norm;
    a1vmul(norm,rvec,uvec);
    idotu = a1vdot(ivec,uvec);
    a1vcross(uvec,isinvec,uxisin);
    dotcos = idotu*(1.-ctheta);
    a1vaxpy(dotcos,uvec,icosvec,ovec);
    a1vadd(ovec,uxisin,ovec);
}

void a1serialno(int *serno)
{
